...
(function (root, factory) {
	...
	function composeQuery(requests, socketId, authOptions) {
		authOptions =
			typeof authOptions === 'function' ? authOptions() : authOptions;
		var i = 0;
		...
		for (var param in authOptions.params) {
			query +=
				...
				'=' +
				encodeURIComponent(authOptions.params[param]);
		}
		return query;
	}
			...
			if (error) {
				...
				objectApply(requests, function (callback, channel) {
					...
					if (responseJson[channel]) {
						if (
							!responseJson[channel].status ||
							responseJson[channel].status === 200
						) {
